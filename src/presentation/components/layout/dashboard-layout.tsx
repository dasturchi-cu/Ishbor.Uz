...
interface NavItem {
  ...
  clientOnly?: boolean
  /** Header AuthenticatedNav da bor — sidebar takrorlanmasin */
  inHeaderNav?: boolean
}

const FREELANCER_NAV: NavItem[] = [
  { id: 'home', href: PATHS.dashboardFreelancer, labelKey: 'nav_dashboard', icon: LayoutDashboard },
  { id: 'services', href: PATHS.dashboardServices, labelKey: 'nav_my_services', icon: Package, freelancerOnly: true, inHeaderNav: true },
  { id: 'applications', href: PATHS.dashboardApplications, labelKey: 'nav_applications', icon: FileText, freelancerOnly: true },
  { id: 'orders', href: PATHS.dashboardOrders, labelKey: 'nav_orders', icon: ShoppingBag, inHeaderNav: true },
  { id: 'messages', href: PATHS.dashboardMessages, labelKey: 'nav_messages', icon: MessageCircle, inHeaderNav: true },
...
const CLIENT_NAV: NavItem[] = [
  ...
  { id: 'find', href: PATHS.services, labelKey: 'nav_find_freelancers', icon: Search, inHeaderNav: true },
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const baseNav = (isClient ? CLIENT_NAV : FREELANCER_NAV).filter((item) => {
    if (isClient && item.freelancerOnly) return false
    if (!isClient && item.clientOnly) return false
    if (item.inHeaderNav) return false
    return true
  })

  const navItems = [
    ...(profile?.is_admin
      ? [{ id: 'admin', href: PATHS.admin, labelKey: 'admin_panel' as TranslationKey, icon: Shield }]
      : []),
    ...baseNav,
  ].map((item) => {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-2" aria-label={t('nav_dashboard')}>
        {navItems.map(({ id, href, labelKey, icon: Icon, badge }) => {
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[calc(100vh-var(--kwork-header-total))] bg-[var(--body-bg)]">
      <div className="flex">
        <aside className="dashboard-sidebar hide-mobile sticky top-[var(--kwork-header-total)] hidden h-[calc(100vh-var(--kwork-header-total))] w-[240px] shrink-0 md:flex md:flex-col">
          <SidebarContent />
        </aside>

        <div className="flex min-w-0 flex-1 flex-col">
          <header className="dashboard-subheader sticky top-[var(--kwork-header-total)] z-40 flex min-h-14 items-center border-b border-[var(--kwork-border)] bg-[var(--neutral-0)] px-4 md:px-6">
          ...
          <div className="dashboard-main flex-1 overflow-y-auto p-4 pb-20 md:p-6 md:pb-6">
      ...
      {mobileOpen && (
        <>
          ...
          <aside
            ...
            className="drawer-panel show-mobile fixed inset-y-0 left-0 z-50 flex flex-col border-r border-[var(--kwork-border)] bg-[var(--neutral-0)] shadow-[var(--shadow-lg)] md:hidden"
          >
...
function DashboardBottomNav() {
  ...
  const items = [
    ...
    {
      href: isClient ? PATHS.services : PATHS.dashboardServices,
      labelKey: (isClient ? 'nav_services' : 'nav_my_services') as TranslationKey,
      icon: isClient ? Search : Package,
    },
  ...
  return (
    <nav
      className="dashboard-bottom-nav show-mobile fixed inset-x-0 bottom-0 z-40 border-t border-[var(--kwork-border)] bg-[var(--neutral-0)] md:hidden"
      aria-label={t('nav_dashboard')}
    >
        ...
        {items.map(({ href, labelKey, icon: Icon, badge }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'relative flex flex-col items-center gap-0.5 px-1 py-2.5 text-[12px] font-medium',
            ...
            >
              ...
              <span className="truncate">{t(labelKey)}</span>
              {badge != null && badge > 0 && (
                <span className="absolute right-2 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--error)] px-1 text-[9px] font-bold text-white">
                  {badge > 9 ? '9+' : badge}
                </span>
              )}
            </Link>
