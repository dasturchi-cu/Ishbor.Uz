...
export function AuthMobileTrust() {
  const { t } = useApp()
  return (
    <ul className="show-mobile mb-5 space-y-2 rounded-lg border border-[var(--kwork-border)] bg-[var(--color-bg-subtle)] p-4">
      {CHECK_KEYS.map((key) => (
        <li key={key} className="flex items-start gap-2 text-[12px] text-[var(--kwork-text-muted)]">
